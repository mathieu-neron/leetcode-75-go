...
func TestLongestZigZag(t *testing.T) {
	t.Skip("TODO: implement longestZigZag")
	tests := []struct {
		tree []any
		want int
	}{
		{[]any{1, nil, 1, 1, 1, nil, nil, 1, 1, nil, 1, nil, nil, nil, 1}, 3},
		{[]any{1, 1, 1, nil, 1, nil, nil, 1, 1, nil, 1}, 4},
		{[]any{1}, 0},
	}
	for _, tc := range tests {
		got := longestZigZag(buildTree(tc.tree))
		if got != tc.want {
			t.Errorf("longestZigZag: got %v, want %v", got, tc.want)
		}
	}
}

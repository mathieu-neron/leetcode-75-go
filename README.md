...
Parallel companion to `leetcode-75-py`. One folder per problem grouped under category folders. Each problem is its own Go package named `solution` (problem 01 uses `mergestrings` from the original seed — same effect, different name).
...
Stubs use `t.Skip` so unimplemented problems don't break the suite. Remove the `t.Skip(...)` line in `solution_test.go` once you start a problem.

## Layout

```
<category>/<NN>-<kebab-slug>/
  solution.go        // function or class stub, returns zero values
  solution_test.go   // t.Skip + scratch space for table-driven cases
```
...
Tree / linked-list problems include the `TreeNode` / `ListNode` struct at the top of `solution.go` — matches how LeetCode pre-fills the editor.
